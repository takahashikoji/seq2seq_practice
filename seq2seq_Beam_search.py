...
def decoder(decoder_inputs, enc_states, dec_cell, decoder_embed_size, vocab_size, 
    ...
    with tf.variable_scope('decoder', reuse=True) :
        ...
        decoder_initial_state = tf.contrib.seq2seq.tile_batch(enc_states, multiplier=10)
        #start_tokens = tf.contrib.seq2seq.GreedyEmbeddingHelper(embed_layer, starting_id_vec, word_to_id['<EOS>'])
        ...
        inference_dec_output, _, _ = tf.contrib.seq2seq.dynamic_decode(inference_decoder, impute_finished=False , 
                                                                                                                              maximum_iterations=max_seq_len)
